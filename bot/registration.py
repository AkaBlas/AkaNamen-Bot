...
def start(update: Update, context: CallbackContext) -> None:
    ...
    """
    text = (
        'Moin! 🙂 Schön, dass Du mitspielen möchtest. Bevor es losgehen kann, muss zuerst '
...
def request_registration(update: Update, context: CallbackContext) -> None:
    ...
    if not guessed_members:
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    text='Akzeptieren', callback_data=ACCEPT_REGISTRATION.format(user.id, '')
                )
            ]
        ]
    else:
        ...
        for i, member in enumerate(guessed_members):
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        text=f'Akzeptiere als Mitglied {i+1}',
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text='Akzeptieren ohne AkaDressen-Daten',
    ...
    context.bot.send_message(
        chat_id=context.bot_data[ADMIN_KEY], text=text, reply_markup=reply_markup
    )
...
def accept_registration_request(update: Update, context: CallbackContext) -> None:
    ...
    text = (
        f'Du bis jetzt mit den folgenden Daten angemeldet: 🥳\n\n{new_member.to_str()}\n\n'
...
def deny_registration_request(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        update.effective_message.edit_text(
            'Der Nutzer hat den Bot inzwischen blockiert. Er '
...
ACCEPT_REGISTRATION_HANDLER = CallbackQueryHandler(
    accept_registration_request, pattern=ACCEPT_REGISTRATION_PATTERN
)
""":class:`telegram.ext.CallbackQueryHandler`: Handler used to parse the acceptance of a
registration request."""
DENY_REGISTRATION_HANDLER = CallbackQueryHandler(
    deny_registration_request, pattern=DENY_REGISTRATION_PATTERN
)
